...
export default function BuyTicket() {
  ...
  const { isConnected, isOnSepolia, login, switchToSepolia, getSigner, getProvider } = useWallet()
  ...
  useEffect(() => {
    if (isConnected && isOnSepolia) fetchEvent()
  }, [isConnected, isOnSepolia, eventId])

  useEffect(() => {
    if (isConnected && isOnSepolia && tier) fetchPrice()
  }, [tier, isConnected, isOnSepolia])
  ...
  if (!isConnected) return (
    <div className="empty-state">
      <h2>Not signed in</h2>
      <p style={{ marginBottom: '1.5rem' }}>Sign in to buy tickets.</p>
